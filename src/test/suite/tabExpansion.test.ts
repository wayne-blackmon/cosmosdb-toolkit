...
suite('Tab Expansion Tests', () => {
  test('sproc snippet expands with Tab', async () => {
    ...
    const item = list.items.find((i) => i.label.toString().startsWith('cosmos.sproc'))
    ...
    const snippetValue =
      insertText instanceof vscode.SnippetString ? insertText.value : (insertText?.toString() ?? '')

    assert.ok(snippetValue.includes('${1'), 'Snippet should contain placeholder tabstops')

    // Verify it's preselected for tab expansion
    assert.ok(item.preselect === true, 'Snippet should be preselected for Tab expansion')
    assert.ok(item.commitCharacters?.includes('\t'), 'Snippet should commit on Tab character')
  })
})
