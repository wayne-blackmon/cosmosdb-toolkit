...
function hasTypeScriptOnlySyntax(line: string): boolean {
  const hasTypedVariableDeclaration =
    /\b(?:const|let|var)\s+[A-Za-z_$][A-Za-z0-9_$]*\s*:\s*[^=;]+(?:=|;|$)/.test(line)
  const hasTypedFunctionParameter =
    /(?:\bfunction\b[^()]*\([^)]*\b[A-Za-z_$][A-Za-z0-9_$]*\s*:\s*[^,)]+)|(?:\([^)]*\b[A-Za-z_$][A-Za-z0-9_$]*\s*:\s*[^,)]+\)\s*=>)/.test(
      line,
    )
  const hasTypedFunctionReturn = /\)\s*:\s*[A-Za-z_$][A-Za-z0-9_$<>,\s\[\]\|&]*\s*(?:=>|\{|$)/.test(
    line,
  )
  ...
  test('All functions have valid JS + TS snippet variants', () => {
    ...
    for (const group of groups) {
      for (const fn of group.functions) {
        ...
        // Validate JS variant
        validateSnippetVariant(fn.snippet.js, `${label}.snippet.js`, failures, {
          allowTypescript: false,
        })

        // Validate TS variant
        validateSnippetVariant(fn.snippet.ts, `${label}.snippet.ts`, failures, {
