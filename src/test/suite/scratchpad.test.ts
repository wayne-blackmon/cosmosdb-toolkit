...
suite('Scratchpad Tests', () => {
  test('openScratchpad opens the scratchpad.ts file', async () => {
    ...
    assert.strictEqual(doc.languageId, 'typescript', 'Scratchpad should open as a TypeScript file')

    assert.ok(doc.uri.fsPath.endsWith('scratchpad.ts'), 'Scratchpad should open the correct file')
  })
})
